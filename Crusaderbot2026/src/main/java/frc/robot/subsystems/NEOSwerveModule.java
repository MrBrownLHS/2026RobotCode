...
public class NEOSwerveModule {
    ...
    private void setSpeed(SwerveModuleState desiredState, boolean isOpenLoop) {
        if(isOpenLoop) {
            ...
            drivePIDController.setSetpoint(desiredState.speedMetersPerSecond, ControlType.kVelocity, ClosedLoopSlot.kSlot0, feedforward.calculate(desiredState.speedMetersPerSecond));
        }
    }

    private void setAngle(SwerveModuleState desiredState) {
        steeringPIDController.setSetpoint(MathUtil.inputModulus(desiredState.angle.getDegrees(), 0, 360), ControlType.kPosition);
